            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <item.icon className={cn('h-4 w-4 shrink-0 transition-colors duration-150', isActive ? 'text-teal-400' : 'text-zinc-500 group-hover:text-zinc-400')} />
                  <AnimatePresence mode="wait">
                    {!collapsed && (
                      <motion.span
      ...
      <div className="relative z-10 p-2 border-t border-white/[0.06]">
        <div className={cn('flex items-center', collapsed ? 'justify-center' : 'justify-end')}>
          <button
            ...
            aria-label={collapsed ? 'Expand sidebar' : isMobile ? 'Close sidebar' : 'Collapse sidebar'}
          ...
          >
            {isMobile ? (
              collapsed ? <ChevronRight className="h-3.5 w-3.5" /> : <X className="h-3.5 w-3.5" />
            ) : (

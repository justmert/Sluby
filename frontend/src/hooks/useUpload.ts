    ...
    try {
      ...
      const upload = new Upload(file, {
            ...
            for (const pair of pairs) {
              ...
              if (key === 'videoAssetId' && b64val) {
                try {
                  ...
                  console.error(
                    '[useUpload] Failed to decode videoAssetId from Upload-Metadata header:',
                    err,
                  );
                }
              }
        ...
        onProgress: (bytesUploaded: number, bytesTotal: number) => {
          ...
          const activeIdx = completedViz < totalVizChunks ? completedViz : totalVizChunks - 1;
          ...
          setState((s) => ({
            ...
            chunks: s.chunks.map((c) => {
              if (c.index < completedViz) return { ...c, status: 'complete' as const };
              if (c.index === activeIdx) return { ...c, status: 'uploading' as const };
        ...
        onSuccess: async () => {
          ...
          if (!videoAssetId && upload.url) {
            try {
              ...
              if (metaHeader) {
                for (const pair of metaHeader.split(',').map((p: string) => p.trim())) {
                  const [key, b64val] = pair.split(' ');
                  if (key === 'videoAssetId' && b64val) {
                    videoAssetId = atob(b64val);
                    setState((s) => ({ ...s, videoAssetId }));
          ...
          if (!videoAssetId) {
            try {
              ...
              const matched = list.data.find((a) => a.title === uploadTitle);
              if (matched) {
          ...
          pollRef.current = setInterval(async () => {
            try {
              ...
              if (err instanceof ApiError && err.status === 404) {
                ...
                setState((s) => ({
                  ...s,
                  error: err instanceof Error ? err.message : 'Processing status check failed',
                }));
              }
            }
          }, POLL_INTERVAL);
